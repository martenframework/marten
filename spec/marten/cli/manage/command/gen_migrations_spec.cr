...
describe Marten::CLI::Manage::Command::GenMigrations do
  ...
  describe "#run" do
    around_each do |t|
      unsynced_app_migrations_path = Marten::CLI::Manage::Command::GenMigrationsSpec::UnsyncedApp::App.new
        .migrations_path

      FileUtils.rm_rf(unsynced_app_migrations_path) if File.exists?(unsynced_app_migrations_path)
    ...
    it "writes migration files without header comments when the --no-header option is used" do
      migrations_path = Marten::CLI::Manage::Command::GenMigrationsSpec::UnsyncedApp::App.new.migrations_path

      command = Marten::CLI::Manage::Command::GenMigrations.new(
        options: ["cli_manage_command_gen_migrations_spec_unsynced_app", "--no-header"] of String,
        stdout: IO::Memory.new,
        stderr: IO::Memory.new
      )
      ...
      migration_files = Dir.glob(migrations_path.join("*.cr").to_s)
      migration_files.size.should eq 1

      content = File.read(migration_files.first)
      content.includes?("Generated by Marten").should be_false
      content.starts_with?("class Migration::").should be_true
    end

    it "writes migration files with header comments when the --no-header option is not used" do
      migrations_path = Marten::CLI::Manage::Command::GenMigrationsSpec::UnsyncedApp::App.new.migrations_path

      command = Marten::CLI::Manage::Command::GenMigrations.new(
        options: ["cli_manage_command_gen_migrations_spec_unsynced_app"] of String,
        stdout: IO::Memory.new,
        stderr: IO::Memory.new
      )
      ...
      content = File.read(migration_files.first)
      content.includes?("Generated by Marten").should be_true
      content.starts_with?("class Migration::").should be_false
    end
  end
end
