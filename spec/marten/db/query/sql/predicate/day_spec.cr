...
describe Marten::DB::Query::SQL::Predicate::Day do
  describe "#to_sql" do
    it "returns the expected SQL statement" do
      ...
      expr = Marten::DB::Query::SQL::Expression::Extract.new(field, "day")
      predicate = Marten::DB::Query::SQL::Predicate::Exact.new(expr, 5, "table")

      for_mysql do
        predicate.to_sql(Marten::DB::Connection.default).should eq(
          {"DAY(table.created_at) = %s", [5]}
        )
      end

      for_postgresql do
        predicate.to_sql(Marten::DB::Connection.default).should eq(
          {"EXTRACT(DAY FROM table.created_at) = %s", [5]}
        )
      end

      for_sqlite do
        predicate.to_sql(Marten::DB::Connection.default).should eq(
          {"CAST(strftime('%d', table.created_at) AS INTEGER) = %s", [5]}
        )
      end
    end
