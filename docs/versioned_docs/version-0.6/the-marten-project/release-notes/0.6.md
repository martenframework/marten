...
As highlighted by the above example, the type of the underlying array member field must be specified through the use of an [`of`](../../schemas/reference/fields.md#of) option, which should reference an [existing schema field type](../../schemas/reference/fields.md#field-types) (such as `string`, `enum`, etc).
...
* Query pages (instances of [`Marten::DB::Query::Page`](pathname:///api/0.5/Marten/DB/Query/Page.html)) now expose a [`#pages_count`](pathname:///api/0.5/Marten/DB/Query/Page.html#pages_count-instance-method) method that allows to get the total number of pages.
...
* A new `#total_count` method was added to the [paginator](pathname:///api/0.5/Marten/DB/Query/Paginator.html#total_count%3AInt32|Int64-instance-method) and [page](pathname:///api/0.5/Marten/DB/Query/Page.html#total_count-instance-method) objects to make it possible to retrieve the total number of records in the paginated query set, without applying pagination.

#### Handlers and HTTP

* [Session stores](../../handlers-and-http/sessions.md) now make it possible to easily configure when session entries expire through the use of the [`#expires_at=`](pathname:///api/0.5/Marten/HTTP/Session/Store/Base.html#expires_at%3D(value%3ATime)-instance-method), [`#expires_at_browser_close=`](pathname:///api/0.5/Marten/HTTP/Session/Store/Base.html#expires_at_browser_close%3D(value%3ABool)-instance-method), and [`#expires_in=`](pathname:///api/0.5/Marten/HTTP/Session/Store/Base.html#expires_in%3D(value%3ATime%3A%3ASpan)-instance-method) methods. Please refer to [Customizing session expiry times](../../handlers-and-http/sessions.md#customizing-session-expiry-times) to learn more about this capability.
* An alias for the [`#reverse`](pathname:///api/0.5/Marten/Handlers/Base.html#reverse(*args%2C**options)-instance-method) handler method ([`#url`](pathname:///api/0.5/Marten/Handlers/Base.html#url(name%3AString|Symbol%2C**kwargs)%3AString-instance-method)) was introduced.
...
* It is now possible to use any hash or named tuple to define schema initial data in handlers that use subclasses of [`Marten::Handlers::Schema`](pathname:///api/0.5/Marten/Handlers/Schema.html) (in the [`#initial_data`](pathname:///api/0.5/Marten/Handlers/Schema.html#initial_data-instance-method) method).
...
* The [`Marten::Handlers::RecordCreate`](../../handlers-and-http/reference/generic-handlers.md#creating-a-record) generic handler now defines a [`#prepare_record_attributes`](pathname:///api/0.5/Marten/Handlers/RecordCreate.html#prepare_record_attributes-instance-method) method that can be overridden in order to generate a hash of attributes that will be used to create the new record (this hash defaults to the validated schema data).
* The [`Marten::Handlers::RecordUpdate`](../../handlers-and-http/reference/generic-handlers.md#updating-a-record) generic handler now defines a [`#save_record`](pathname:///api/0.5/Marten/Handlers/RecordUpdate.html#save_record%3ANil-instance-method) method that can be overridden in order to customize how the record is saved from the validated schema data.
* The [`Marten::Handlers::RecordUpdate`](../../handlers-and-http/reference/generic-handlers.md#updating-a-record) generic handler now defines a [`#prepare_record_attributes`](pathname:///api/0.5/Marten/Handlers/RecordUpdate.html#prepare_record_attributes-instance-method) method that can be overridden in order to generate a hash of attributes that will be used to update the record (this hash defaults to the validated schema data fields that match the model's fields).
