...
Defining an emailing backend is as simple as creating a class that inherits from the [`Marten::Emailing::Backend::Base`](pathname:///api/0.7/Marten/Emailing/Backend/Base.html) abstract class and that implements a unique `#deliver` method. This method takes a single `email` argument (instance of [`Marten::Emailing::Email`](pathname:///api/0.7/Marten/Emailing/Email.html)), corresponding to the email to deliver.
...
If your backend supports attachments, you can iterate over [`#attachments`](pathname:///api/0.7/Marten/Emailing/Email.html) in the `#deliver` implementation:
