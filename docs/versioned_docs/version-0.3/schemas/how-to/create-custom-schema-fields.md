...
To do so, you will have to call the [`Marten::Schema::Field#register`](pathname:///api/0.3/Marten/Schema/Field.html#register(id%2Cfield_klass)-macro) method with the identifier of the field you wish to use, and the actual field class. For example:
...
The default `#initialize` method that is provided by the [`Marten::Schema::Field::Base`](pathname:///api/0.3/Marten/Schema/Field/Base.html) is fairly simply and looks like this:
