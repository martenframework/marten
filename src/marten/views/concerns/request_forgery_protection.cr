module Marten
  module Views
    ...
    #
    # By default, views will use the CSRF protection by complying with what is defined as part of the
    # `Marten.settins.csrf.protection_enabled` setting (whose value is `true` by default). It is also possible to
    # override whether or not CSRF protection is used on a per-view basis by using the `#protect_from_forgery` method.
    module RequestForgeryProtection
      ...
      macro included
        ...
        extend Marten::Views::RequestForgeryProtection::ClassMethods
      ...
      module ClassMethods
        # Allows to define whether or not the view should be protected from Cross-Site Request Forgeries.
        def protect_from_forgery(protect : Bool) : Nil
          @@protect_from_forgery = protect
        end

        # Returns a boolean indicating if the view is protected from Cross-Site Request Forgeries.
        def protect_from_forgery?
          @@protect_from_forgery.nil? ? Marten.settings.csrf.protection_enabled : @@protect_from_forgery
        end
      end

      # Returns a valid CSRF token to use in the context of the current view instance.
      #
      ...
      private def protect_from_forgery
        ...
        # safe if their defined semantics are essentially read-only) or if CSRF protection is disable on a per-view
        # basis.
