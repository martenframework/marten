...
It is now possible to generate streaming responses from iterators of strings easily by leveraging the [`Marten::HTTP::Response::Streaming`](https://crystaldoc.info/github/martenframework/marten/v0.3.0/Marten/HTTP/Response/Streaming.html) class or the [`#respond`](https://crystaldoc.info/github/martenframework/marten/v0.3.0/Marten/Handlers/Base.html#respond(streamed_content%3AIterator(String)%2Ccontent_type%3DHTTP%3A%3AResponse%3A%3ADEFAULT_CONTENT_TYPE%2Cstatus%3D200)-instance-method) helper method. This can be beneficial if you intend to generate lengthy responses or responses that consume excessive memory (a classic example of this is the generation of large CSV files).
...
The global cache can be accessed by leveraging the [`Marten#cache`](https://crystaldoc.info/github/martenframework/marten/v0.3.0/Marten.html#cache%3ACache%3A%3AStore%3A%3ABase-class-method) method. Here are a few examples on how to perform some basic caching operations:
...
* An [`#any?`](https://crystaldoc.info/github/martenframework/marten/v0.3.0/Marten/DB/Query/Set.html#any%3F-instance-method) method was added to query sets in order to short-circuit the default implementation of [`Enumerable#any?`](https://crystal-lang.org/api/Enumerable.html#any%3F%3ABool-instance-method) and to avoid loading the full list of records in memory (when called without arguments). This overridden method is technically an alias of [`#exists?`](../../models-and-databases/reference/query-set.md#exists).
...
* The [`Marten::Handlers::RecordList`](../../handlers-and-http/reference/generic-handlers.md#listing-records) generic record now provides the ability to specify a custom [query set](../../models-and-databases/queries.md) instead of a model class. This can be achieved through the use of the [`#queryset`](https://crystaldoc.info/github/martenframework/marten/v0.3.0/Marten/Handlers/RecordListing.html#queryset(queryset)-macro) macro.
...
* [`Marten::HTTP::Errors::SuspiciousOperation`](https://crystaldoc.info/github/martenframework/marten/v0.3.0/Marten/HTTP/Errors/SuspiciousOperation.html) exceptions are now showcased using the debug internal error page handler to make it easier to diagnose errors such as unexpected host errors (which result from a missing host value in the [`allowed_hosts`](../../development/reference/settings.md#allowed_hosts) setting).
* [`Marten#setup`](https://crystaldoc.info/github/martenframework/marten/v0.3.0/Marten.html#setup-class-method) now raises.[`Marten::Conf::Errors::InvalidConfiguration`](https://crystaldoc.info/github/martenframework/marten/v0.3.0/Marten/Conf/Errors/InvalidConfiguration.html) exceptions when a configured database involves a backend that is not installed (eg. a MySQL database configured without `crystal-lang/crystal-mysql` installed and required).
...
* It is now possible to specify command aliases when defining management commands by leveraging the [`#command_aliases`](https://crystaldoc.info/github/martenframework/marten/v0.3.0/Marten/CLI/Manage/Command/Base.html#command_aliases(*aliases%3AString|Symbol)-class-method) helper method.
...
* [Custom route parameter](../../handlers-and-http/how-to/create-custom-route-parameters.md) must now implement a [`#regex`](https://crystaldoc.info/github/martenframework/marten/v0.3.0/Marten/Routing/Parameter/Base.html#regex%3ARegex-instance-method) method and can no longer rely on a `#regex` macro to generate such method.
