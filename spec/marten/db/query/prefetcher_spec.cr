    ...
    context "with a non-default database" do
      ...
      it "uses a custom query set that filters records when prefetching" do
        ...
        prefetcher = Marten::DB::Query::Prefetcher.new(
          ...
          using: nil,
          custom_query_sets: {
            "authors" => Marten::DB::Query::PrefetcherSpec::Author.filter(name__contains: "Muster").as(Marten::DB::Query::Set::AnyQuerySet)
          }
        )
      ...
      it "doesn't use a custom query if none is specified for that relation" do
        ...
        prefetcher = Marten::DB::Query::Prefetcher.new(
          ...
          using: nil,
          custom_query_sets: {
            "conferences" => Marten::DB::Query::PrefetcherSpec::Conference.filter(name__contains: "Crystal").as(Marten::DB::Query::Set::AnyQuerySet)
          }
        )
      ...
      it "raises if an incompatible query set is used for the custom query" do
        ...
        prefetcher = Marten::DB::Query::Prefetcher.new(
          ...
          using: nil,
          custom_query_sets: {
            "authors" => Marten::DB::Query::PrefetcherSpec::Bio.filter(content__contains: "Muster").as(Marten::DB::Query::Set::AnyQuerySet)
          }
        )
