...
* by using the [assets engine](pathname:///api/0.6/Marten/Asset/Engine.html#url(filepath%3AString)%3AString-instance-method) in Crystal
...
It is also possible to resolve asset URLs programmatically in Crystal. To do so, you can leverage the [`#url`](pathname:///api/0.6/Marten/Asset/Engine.html#url(filepath%3AString)%3AString-instance-method) method of the Marten assets engine:
