...
describe Marten::HTTP::Cookies do
  ...
  describe "#fetch_signed" do
    it "allows to retrieve a specific signed cookie value using its name" do
      cookies = Marten::HTTP::Cookies.new
      cookies.set_signed("test", "value")
      cookies.fetch_signed("test") { "fallback" }.should eq "value"
    end
    ...
    it "yields the cookie name when not found" do
      cookies = Marten::HTTP::Cookies.new
      cookies.set_signed("test", "value")
      cookies.fetch_signed("unknown") { |n| n }.should eq "unknown"
    end

    it "returns the default value if the cookie name is not found" do
      cookies = Marten::HTTP::Cookies.new(HTTP::Cookies{"test" => "value"})
      cookies.set_signed("test", "value")
      cookies.fetch_signed("unknownn", "fallback").should eq "fallback"
    end
  end
  ...
  describe "#set_signed" do
    it "allows to set a simple cookie using a cookie name string" do
      cookies = Marten::HTTP::Cookies.new
      cookies.set_signed("foo", "bar")
      cookies["foo"].should_not eq "bar"
      cookies.fetch_signed("foo").should eq "bar"
    end
    ...
    it "allows to set a simple cookie associated with an expiry time" do
      ...
      cookies.set_signed("foo", "bar", expires: expiry)
    ...
    it "allows to set a simple cookie associated with a custom path" do
      ...
      cookies.set_signed("foo", "bar", path: "/path")
    ...
    it "allows to set a simple cookie associated with a domain" do
      ...
      cookies.set_signed("foo", "bar", domain: "example.com")
    ...
    it "allows to set a secure cookie" do
      ...
      cookies.set_signed("foo", "bar", secure: true)
    ...
    it "allows to set a HTTP-only cookie" do
      ...
      cookies.set_signed("foo", "bar", http_only: true)
    ...
    it "allows to set a lax cookie" do
      ...
      cookies.set_signed("foo", "bar", same_site: "lax")
    ...
    it "allows to set a strict cookie" do
      ...
      cookies.set_signed("foo", "bar", same_site: :strict)
