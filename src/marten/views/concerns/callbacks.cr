module Marten
  module Views
    # Provides the ability to define view callbacks.
    #
    # This module provides the ability to define callbacks that are executed before and / or after dispatching the
    # considered views. They allow to intercept the incoming request and to return early HTTP responses. Two hooks are
    # enabled by the use of this module: `before_dispatch` callbacks are executed before the execution of the view's
    ...
    module Callbacks
      ...
      # Allows to do define callbacks that are called before executing `Marten::View#dispatch`.
      #
      # Those callbacks have access to the incoming `#request` object and they can return early response. If one of such
      # callbacks returns an early response, the following callbacks will be skipped, as well as the view's `#dispatch`
      # method.
      ...
      # Allows to do define callbacks that are called after executing `Marten::View#dispatch`.
      #
      ...
      # response will be returned by the view instead of the one returned by the `#dispatch` method.
