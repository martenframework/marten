...
:::info
The `name` parameter for included routes is optional, i.e. `path "/articles", Articles::ROUTES` is also valid. Please note that this will increase the possibility of a name collision and it is therefore advisable to prefix the individual paths of the included route,
