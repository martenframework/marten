...
Marten comes with a built-in command line interface (CLI) that developers can leverage in order to perform common actions and to interact with the framework. This tool provides a set of built-in sub-commands that can be easily extended with new commands.
...
As you can see, the `marten` CLI must be used with a specific **command**, possibly followed by **options** and **arguments** (which may be required or not depending on the considered command). All the built-in commands are listed in [management commands reference](./reference/management-commands).
