...
Let's start by creating a first view for your project. To do so, create a `src/views/home_view.cr` file with the following content:
...
Now if you go to [http://localhost:8000](http://localhost:8000), you will get the `Hello World!` response that is generated by the view you just wrote.

:::info
Please refer to [Routing](../views-and-http/routing) to learn more about Marten's routing mechanism.
:::
...
Let's revisit our initial implementation of the `HomeView` view we defined [earlier](#writing-a-first-view).

Since we are building a weblog application, it would make sense to display a list of all our `Article` objects on the index page. To do so, let's update the existing `src/views/home_view.cr` file with the following content:
...
Let's define the expected template for our home view by creating a `src/templates/home.html` file with the following content:
...
We have now pieced together the main components of the Marten web framework (Models, Views, Templates). When accessing the home page of our application, the following steps are taken care of by the framework:
...
3. the Marten routing system maps the path of the incoming request to the `HomeView` view
4. the view is initialized and executed, which involves fetching all the `Article` records
5. the view renders the `home.html` template and returns a `200 OK` response containing the rendered content
6. the Marten server sends the response with the HTML content back to the browser

## Showing a single article

We presently have a view that lists all the existing `Article` records, but it would be nice to be able to actually see the content of each article individually.

To do so, let's create a new `src/views/article_detail_view.cr` view file with the following content:

```crystal title="src/views/article_detail_view.cr"
class ArticleDetailView < Marten::View
  def get
...
Let's now map this view to a new route by adding the following line to the `config/routes.cr` file:

```crystal title="config/routes.cr"
Marten.routes.draw do
  path "/", HomeView, name: "home"
  // highlight-next-line
  path "/article/<pk:int>", ArticleDetailView, name: "article_detail"
...
As you can see above, the new route we mapped to the `ArticleDetailView` view requires a `pk` - _primary key_ - integer (`int`) parameter. Route parameters are defined using angle brackets, and the name of the parameter and its type are separated by a `:` character (`<name:type>` format).
...
:::info
Please refer to [Routing](../views-and-http/routing) to learn more about Marten's routing system.
:::
...
Let's see how we can use this schema in a view. In this light, let's create a new `src/views/article_create_view.cr` file:

```crystal title="src/views/article_create_view.cr"
...
This view is able to handle both GET and POST requests:
...
Finally, we need to map the `ArticleCreateView` view to a proper route. We can do this by editing the `config/routes.cr` file as follows:
...
Now that we are able to create new `Article` records, let's add the ability to update existing records. To do so, we will implement a view that works similarly to the `ArticleCreateView` view we defined earlier: it should be able to process GET requests in order to display an "update" form, and it should validate the incoming data (and update the right record) when POST requests are submitted through an HTML form.

In this light, let's define the `src/views/article_update_view.cr` file with the following content:

```crystal title="src/views/article_update_view.cr"
class ArticleUpdateView < Marten::View
  @article : Article?
  @schema : ArticleSchema?
...
Let's now map the `ArticleUpdateView` view to a proper route. We can do this by editing the `config/routes.cr` file as follows:
...
Marten.routes.draw do
  ...
  path "/article/<pk:int>", ArticleDetailView, name: "article_detail"
  // highlight-next-line
  path "/article/<pk:int>/update", ArticleUpdateView, name: "article_update"
...
Finally, the last missing feature we could add is the ability to delete an article. To do so, let's introduce a view that works as follows: when processing a GET request the view will ask the user to confirm that they want to indeed delete the considered `Article` record, and when processing a POST request the view will actually delete the record and then redirect to the home page of the application.

In this light, let's define the `src/views/article_delete_view.cr` file:

```crystal title="src/views/article_delete_view.cr"
class ArticleDeleteView < Marten::View
  @article : Article?
...
Let's now map the `ArticleDeleteView` view to a proper route. We can do this by editing the `config/routes.cr` file as follows:
...
Marten.routes.draw do
  ...
  path "/article/<pk:int>/update", ArticleUpdateView, name: "article_update"
  // highlight-next-line
  path "/article/<pk:int>/delete", ArticleDeleteView, name: "article_delete"
...
The views we implemented previously map to common web development use cases: retrieving data from the database - from a specific URL paramater - and displaying it, listing multiple objects, creating or updating records, etc. These use cases are so frequently encountered that Marten provides a set of "generic views" that allow to easily implement them. These generic views take care of these common patterns so that developers don't end up reimplementing the wheel.

We could definitely leverage these generic views as part of our weblog application.

In this light, let's start with the `HomeView` class we implemented earlier: this view essentially retrieves all the `Article` records and makes this list available to the `home.html` template. This pattern is enabled by the [`Marten::Views::RecordList`](pathname:///api/Marten/Views/RecordList.html) generic view. In order to use it, let's modify the `src/views/home_view.cr` file as follows:

```crystal title="src/views/home_view.cr"
class HomeView < Marten::Views::RecordList
  model Article
...
In the above snippet we use a few class methods in order to define how the view should behave: `#model` allows to define the model class that should be used to retrieve the record, `#template_name` allows to define the name of the template to render, and `#list_context_name` allows to define the name of the record list variable in the template context.

Let's continue with the `ArticleDetailView` class: this view retrieves a specific `Article` record from a `pk` route parameter, and "renders" it using a specific template. This pattern is enabled by the [`Marten::Views::RecordDetail`](pathname:///api/Marten/Views/RecordDetail.html) generic view. In order to use it, let's modify the `src/views/article_detail_view.cr` file as follows:
...
In order to configure how the view should behave, we make use of a few class methods here as well: `#model` allows to define the model class of the record that should be retrieved, `#template_name` defines the template to render, and `#record_context_name` defines the name of the record variable in the template context.

Now let's look at the `ArticleCreateView` class: this class displays a form when processing GET requests, and it validates a schema that is used to create a specific record when processing POST requests. This exact pattern is enabled by the [`Marten::Views::RecordCreate`](pathname:///api/Marten/Views/RecordCreate.html) generic view. In order to use it, we can modify the `src/views/article_create_view.cr` file as follows:

```crystal title="src/views/article_create_view.cr"
class ArticleCreateView < Marten::Views::RecordCreate
  model Article
  schema ArticleSchema
...
We can now look at the `ArticleUpdateView` class: this class retrieves a specific record and displays a form when processing GET requests, and it validates a schema whose data is used to update the record when processing POST requests. This pattern is enabled by the [`Marten::Views::RecordUpdate`](pathname:///api/Marten/Views/RecordUpdate.html) generic view. Let's use it and let's modify the `src/views/article_update_view.cr` file as follows:

```crystal title="src/views/article_update_view.cr"
class ArticleUpdateView < Marten::Views::RecordUpdate
  model Article
  schema ArticleSchema
...
Finally, let's look at the `ArticleDeleteView` class: this view renders a template when processing GET requests, and performs the deletion of the considered record when processing POST requests. This pattern is provided by the [`Marten::Views::RecordDelete`](pathname:///api/Marten/Views/RecordDelete.html) generic view. In order to use it, let's modify the `src/views/article_delete_view.cr` file as follows:
...
:::info
Please refer to [Generic views](../views-and-http/generic-views) to learn more about generic views.
:::
...
* we learned to create [views](../views-and-http) and to map URLs to these in order to process HTTP requests
