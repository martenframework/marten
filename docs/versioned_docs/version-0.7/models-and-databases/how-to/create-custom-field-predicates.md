...
Field predicates are subclasses of the [`Marten::DB::Query::SQL::Predicate::Base`](pathname:///api/0.7/Marten/DB/Query/SQL/Predicate/Base.html) abstract class. Each predicate class is responsible for:
...
The lookup name is defined with [`#predicate_name`](pathname:///api/0.7/Marten/DB/Query/SQL/Predicate/Base.html#predicate_name(name%3AString%3F)-class-method). The SQL is generated by implementing [`#to_sql`](pathname:///api/0.7/Marten/DB/Query/SQL/Predicate/Base.html#to_sql(connection%3AConnection%3A%3ABase)-instance-method), which receives the current database connection and must return a tuple of `{sql, params}`.
...
To do so, call [`Marten::DB::Query::SQL::Predicate#register`](pathname:///api/0.7/Marten/DB/Query/SQL/Predicate.html#register(predicate_klass%3ABase.class)-class-method) with the predicate class:
...
`#sql_params` converts the filter value through the field. You can map over those parameters when the bound value should differ from the value passed to `#filter` — for example to wrap a string for a `LIKE` pattern, using [`#sanitize_like_pattern`](pathname:///api/0.7/Marten/DB/Connection/Base.html#sanitize_like_pattern(pattern%3AString)%3AString-instance-method) to escape `%` and `_`:
