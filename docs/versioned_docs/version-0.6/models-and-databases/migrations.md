...
Migrations always inherit from the [`Marten::Migration`](pathname:///api/0.6/Marten/DB/Migration.html) base class. A basic migration will look something like this:
...
It is possible to disable this default behavior by using the [`#atomic`](pathname:///api/0.6/Marten/DB/Migration.html#atomic(atomic%3ABool)-class-method) method, in the migration class:
...
If we look at the generated migration, you will notice that it includes multiple calls to the [`#replaces`](pathname:///api/0.6/Marten/DB/Migration.html#replaces(app_name%3AString|Symbol%2Cmigration_name%3AString|Symbol)-class-method) class method:
