...
The global cache can be accessed by leveraging the [`Marten#cache`](pathname:///api/0.7/Marten.html#cache%3ACache%3A%3AStore%3A%3ABase-class-method) method. Here are a few examples on how to perform some basic caching operations:
...
* An [`#any?`](pathname:///api/0.7/Marten/DB/Query/Set.html#any%3F-instance-method) method was added to query sets in order to short-circuit the default implementation of [`Enumerable#any?`](https://crystal-lang.org/api/Enumerable.html#any%3F%3ABool-instance-method) and to avoid loading the full list of records in memory (when called without arguments). This overridden method is technically an alias of [`#exists?`](../../models-and-databases/reference/query-set.md#exists).
...
* [`Marten::HTTP::Errors::SuspiciousOperation`](pathname:///api/0.7/Marten/HTTP/Errors/SuspiciousOperation.html) exceptions are now showcased using the debug internal error page handler to make it easier to diagnose errors such as unexpected host errors (which result from a missing host value in the [`allowed_hosts`](../../development/reference/settings.md#allowed_hosts) setting).
* [`Marten#setup`](pathname:///api/0.7/Marten.html#setup-class-method) now raises.[`Marten::Conf::Errors::InvalidConfiguration`](pathname:///api/0.7/Marten/Conf/Errors/InvalidConfiguration.html) exceptions when a configured database involves a backend that is not installed (eg. a MySQL database configured without `crystal-lang/crystal-mysql` installed and required).
...
* It is now possible to specify command aliases when defining management commands by leveraging the [`#command_aliases`](pathname:///api/0.7/Marten/CLI/Manage/Command/Base.html#command_aliases(*aliases%3AString|Symbol)-class-method) helper method.
...
* [Custom route parameter](../../handlers-and-http/how-to/create-custom-route-parameters.md) must now implement a [`#regex`](pathname:///api/0.7/Marten/Routing/Parameter/Base.html#regex%3ARegex-instance-method) method and can no longer rely on a `#regex` macro to generate such method.
